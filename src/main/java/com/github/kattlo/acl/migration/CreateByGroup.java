...
public class CreateByGroup implements Strategy {
    ...
    private List<AclBinding> topicBindingsFor(String principal, JSONObject access,
        ...
        var groupName = group.getString(ID_ATTRIBUTE);

        var operationsJson = JSONPointer.asArray(group, OPERATIONS_RELATIVE_POINTER);
        log.debug("Operations to {}: {}", permission, operationsJson);

        var pattern = patternOf(groupName, ResourceType.GROUP);
        log.debug("Group ResourcePattern {}", pattern);
